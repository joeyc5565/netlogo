...
breed [anabaenas anabaena]
breed [chlorogoniums chlorogonium]
breed [nitzschias nitzschia]
breed [scenedesmuss scenedesmus]
breed [trachelomonass trachelomonas]
...
anabaenas-own [
    ...
    ;; and assigned based off species when the turtles are created.
...
to setup
  ...
  ask scenedesmuss [
   ...
   ask trachelomonass [
    ...
    create-scenedesmuss 5 [
      setxy random-xcor random-ycor
    ]

    create-trachelomonass 5 [
      setxy random-xcor random-ycor
    ]
end

to normalcdf [z]  ;; normal cumulative distribution function  (lb = abs(z), ub = inf, mean = 0, stdev = 1)
...
to go
    ask turtles [
        ...
        let phosphorus-value [phosphorus] of patch-here
        let phosphate-value [phosphate] of patch-here
        let ammonia-value [ammonia] of patch-here
        let kjeldahl-nitrogen-value [kjeldahl-nitrogen] of patch-here
        let chlorophyll-a-value [chlorophyll-a] of patch-here
        let nitrogen-phosphorus-ratio-value [nitrogen-phosphorus-ratio] of patch-here
        let calcium-carbonate-value [calcium-carbonate] of patch-here
        let temperature-value [temperature] of patch-here
        let ph-value [ph] of patch-here
        let dissolved-oxygen-value [dissolved-oxygen] of patch-here
        let turbidity-value [turbidity] of patch-here
        ...
        let proportion-phosphorus normalcdf (abs (phosphorus-value - mean-phosphorus) / stdev-phosphorus)
        let proportion-phosphate normalcdf (abs (phosphate-value - mean-phosphate) / stdev-phosphate)
